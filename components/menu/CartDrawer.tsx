...
export default function CartDrawer({ open, onClose, theme }: CartDrawerProps) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b" style={{ borderColor: `${theme.accentHex}20` }}>
          <div className="flex items-center gap-3">
            <ShoppingBag size={18} style={{ color: theme.accentHex }} />
            <span className="font-cinzel text-[14px] tracking-[0.12em] uppercase" style={{ color: theme.accentHex }}>
              Your Selection
            </span>
          </div>
          ...
          {items.length === 0 ? (
            ...
            items.map((item: any) => (
              <div
                ...
                style={{ background: `${theme.accentHex}08`, border: `1px solid ${theme.accentHex}18` }}
              >
                {/* Veg indicator */}
                <div className={`mt-0.5 w-[10px] h-[10px] rounded-[2px] border flex-shrink-0 flex items-center justify-center ${item.isVeg ? "border-green-600" : "border-red-600"}`}>
                  <div className={`w-[4px] h-[4px] rounded-full ${item.isVeg ? "bg-green-600" : "bg-red-600"}`} />
                </div>

                {/* Details */}
                <div className="flex-1 min-w-0">
                  <p className="font-cormorant text-[13px] font-semibold text-white/90 leading-tight">
                    {item.name}
                  </p>
                  {item.description && (
                    <p className="font-cormorant text-[11px] text-white/35 truncate mt-0.5">{item.description}</p>
                  )}
                  <p className="font-cormorant text-[12px] font-semibold mt-1" style={{ color: theme.accentHex }}>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button
                    ...
                    className="w-6 h-6 rounded-full border flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity"
                    style={{ borderColor: `${theme.accentHex}40`, color: theme.accentHex }}
                  >
                    <Minus size={10} />
                  </button>
                  <span className="font-cormorant text-[13px] font-semibold text-white/80 w-4 text-center">
                    {item.quantity}
                  </span>
                  <button
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="px-5 py-5 border-t flex flex-col gap-3" style={{ borderColor: `${theme.accentHex}20` }}>
            <div className="flex items-center justify-between">
              <span className="font-cinzel text-[11px] tracking-[0.12em] text-white/40 uppercase">Total</span>
              <span className="font-cormorant text-[20px] font-semibold" style={{ color: theme.accentHex }}>
            ...
            >
              ...
              <div className="flex flex-col gap-1.5 mt-2">
                {items.map((item: any) => (
                  <p key={item.name} className="font-cormorant text-[13px] text-white/70">
